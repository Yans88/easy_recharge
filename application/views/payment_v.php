...
<div class="modal fade" role="dialog" id="frm_user">
          <div class="modal-dialog" style="width:400px">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">×</span></button>
                <h4 class="modal-title">Add Payment Channel</h4>
              </div>
              ...
              <div class="modal-body" style="padding-bottom:2px;">
                ...
                <div class="form-group">
                  <label>Kode Payment</label><span class="label label-danger pull-right kode_payment_error"></span>
                  <input type="text" class="form-control" name="kode_payment" id="kode_payment" value="" placeholder="Kode Payment" autocomplete="off" />
                  <input type="hidden" value="" name="id_payment" id="id_payment">
                </div>
				
				<div class="form-group">
                  <label>Payment Name</label><span class="label label-danger pull-right payment_name_error"></span>
                  <input type="text" class="form-control" name="payment_name" id="payment_name" value="" placeholder="Payment Name" autocomplete="off" />
                </div>	
				...
				<div class="form-group">
                  <label>Convenience Fee</label><span class="label label-danger pull-right convenience_fee_error"></span>
                  <input type="text" class="form-control" name="convenience_fee" id="convenience_fee" value="" placeholder="Convenience Fee" autocomplete="off" />
                </div>	
 ...
 <div class="box-header">
    <a href="#"><button class="btn btn-success add_user"><i class="fa fa-plus"></i> Add Payment Channel</button></a>
</div>
<div class="box-body">
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
			...
			<th style="text-align:center; width:30%">Kode Payment</th>
			<th style="text-align:center; width:30%">Payment Name</th>
			<th style="text-align:center; width:20%">Convenience Fee</th>
			...
			<?php 
				...
				if(!empty($master_payment)){
					foreach($master_payment as $mp){
						...
						echo '<td style="vertical-align: middle;">'.$mp['kode_payment'].'</td>';
						echo '<td style="vertical-align: middle;">'.ucwords($mp['nama_payment']).'</td>';		
...
$('.yes_del').click(function(){
	var id = $('#del_id').val();
	var url = '<?php echo site_url('payment_channel/del');?>';
	$.ajax({
		data : {id : id},
		url : url,
		type : "POST",
...
$('.yes_save').click(function(){
	...
	var kode_payment = $('#kode_payment').val();
	...
	if(kode_payment <= 0 || kode_payment == '') {
		$('.kode_payment_error').text('Kode Payment harus diisi');
		return false;
	}
	if(payment_name <= 0 || payment_name == '') {
		$('.payment_name_error').text('Payment Name harus diisi');
		return false;
	}
	if(type <= 0 || type == '') {
	...
	if(convenience_fee <= 0 || convenience_fee == '') {
		$('.convenience_fee_error').text('Convenience Fee harus diisi');
		return false;
	}
	var dt = $('#form_users').serialize();
	var url = '<?php echo site_url('payment_channel/save');?>';
	$.ajax({
		data:dt,
		type:'POST',
		url : url,
...
$('.add_user').click(function(){
	...
	$('#kode_payment').prop('disabled', false);
	$('.payment_name_error').text('');
	$('.kode_payment_error').text('');
	$('.type_error').text('');	
	$('#frm_user').modal({
