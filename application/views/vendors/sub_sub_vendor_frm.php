...
<div class="box box-success">
                            ...
                            <tr style="vertical-align:middle;">
                                ...
                                <td colspan="4">
                                    <input type="hidden" name="id_vendor" id="id_vendor"
                                           value="<?php echo !empty($vendors) ? $vendors->id_sub_vendor : ''; ?>"/>
                                    <input type="hidden" name="id_owner" id="id_owner"
                                           value="<?php echo !empty($owner) ? $owner->id_sub_vendor : ''; ?>"/>
    ...
    <div class="box-footer" style="height:35px;">
        ...
        <div class="pull-right">
            <a href="<?php echo site_url('vendors/list_sub_vendor/' . $owner->id_sub_vendor); ?>">
...
<script type="text/javascript">
    ...
    $('.btn_save').click(function () {
        ...
        var sharingUpline = '<?php echo (int)$owner->sharing;?>';
        var profit = $('#profit').val();
        var sharing = $('#sharing').val();
        console.log(sharingUpline);
        $('.profit_error').text('');
        if (profit <= 0 || profit == '') {
            $('.profit_error').text('Harus diisi minimal 1 dan maksimal 100');
            return false;
        }
        if (Number(profit) > 100) {
            $('.profit_error').text('Jumlah profit dan sharing tidak boleh lebih dari 100%');
            return false;
        }
        if (Number(profit) > Number(sharingUpline)) {
            $('.profit_error').text('Jumlah profit tidak boleh lebih dari ' + Number(sharingUpline) + '%');
            return false;
        }
        var url = '<?php echo site_url('vendors/simpan_sub_sub_vendor');?>';
        $.ajax({
            url: url,
            data: dt,
            type: 'POST',
            success: function (result) {
                if (result > 0) {
                    ...
                    window.location = '<?php echo site_url('vendors/list_sub_sub_vendor/' . $owner->id_sub_vendor);?>';
                }
            }
        });
    });
