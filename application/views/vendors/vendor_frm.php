                    ...
                    <td>
                        <table class="table table-responsive">
                            <tr style="vertical-align:middle;">
                                <td width="11%"><b>Nama Vendor </b><br/><span
                                            class="label label-danger nama_vendor_error"></span></td>
                                <td width="2%">:</td>
                                <td colspan="4">
                                    <input type="hidden" name="id_vendor" id="id_vendor"
                                           value="<?php echo !empty($vendors) ? $vendors->id_vendor : ''; ?>"/>
...
<script type="text/javascript">
    ...
    $('.btn_save').click(function () {
        ...
        $('.nama_vendor_error').text('');
        if (nama_vendor == '') {
            $('.nama_vendor_error').text('Harus diisi minimal 1 dan maksimal 100');
            return false;
        }
        var url = '<?php echo site_url('vendors/simpan_vendor');?>';
        $.ajax({
            url: url,
            data: dt,
            type: 'POST',
            success: function (result) {
                if (result > 0) {
                    ...
                    window.location = '<?php echo site_url('vendors/list_vendor');?>';
                }
            }
        });
    });
