...
<div class="box box-success">
    <div class="box-header">
        <a href="<?php echo site_url('vendors/add_sub_vendor/' . $id_vendor); ?>">
    ...
    <div class="box-body">
        ...
        <table id="example88" class="table table-bordered table-striped">
            ...
            <tr>
                ...
                <th style="text-align:center; width:10%">Sharing</th>
                <th style="text-align:center; width:37%">Address</th>
            ...
            if (!empty($vendors)) {
                foreach ($vendors as $m) {
                    $view_sub = '';
                    $info = $m['id_vendor'] . 'Þ' . $m['nama_vendor'];
                    echo '<tr>';
                    echo '<td align="center">' . $i++ . '.</td>';
                    echo '<td>' . $m['nama_vendor'] . '</td>';
                    echo '<td>' . $m['profit'] . '</td>';
                    echo '<td>' . $m['sharing'] . '</td>';
                    echo '<td>' . $m['address'] . '</td>';
                    echo '<td align="center" style="vertical-align: middle;">';

                    echo '
			<a href="' . site_url('vendors/list_sub_sub_vendor/' . $m['id_sub_vendor']) . '" title="List Sub sub Vendor"><button class="btn btn-xs btn-primary"><i class="fa fa-list"></i> List Sub sub Vendor</button></a>
			<a href="' . site_url('vendors/add_sub_vendor/' . $m['id_vendor'] . '/' . $m['id_sub_vendor']) . '" title="Edit"><button class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</button></a>
			<button title="Delete" id="' . $m['id_sub_vendor'] . '" class="btn btn-xs btn-danger del_news"><i class="fa fa-trash-o"></i> Delete</button>		
						</td>';
                    echo '</tr>';
                }
            }
